...
授業形態は、通信状態とは無関係な2モードです。

- オンライン授業モード：Style Research active 8自治体では、
  `item_image_mapping_pilot_top8.csv` の `VERIFIED` 76組だけを画像問題として表示する。
  学習者画面は品目画像、正式区分名付きの箱、正誤判定だけを表示する。
- 対面授業モード：画像問題を表示せず、自治体の分別箱だけを投影する。

オンライン授業モードでも、品目名、条件文、正解後の公式ルール補足は学習者画面へ
表示しません。これらの情報は教師用データとして保持します。

- `UNRESOLVED` 4組は出題・正誤判定から除外する。
- M098尾道市・M099福山市は地域variant対応前のため画像問題へ含めない。
- `REFERENCE_ONLY` の公式下位区分はcategory正本の親子関係をたどり、最初の
  CURRENT `SORT_BUCKET`へ投影する。公式下位区分の情報はデータ側に保持する。
- Style Researchが`OFFICIAL_CONFIRMED` / `OFFICIAL_DERIVED`の箱だけ公式色を使う。
  `NOT_CONFIRMED`は白い標準表示とし、公式色とは表示上も区別する。
- 色だけを唯一の手掛かりにせず、箱の正式名称・枠線・フォーカス表示を併用する。
