#!/usr/bin/env python3
"""Apply the official item-mapping pilot for 10 image items and 9 municipalities.
...
R: dict[str, dict[str, tuple[str, str, str, str, str, str, str, str]]] = {
    ...
    "M107": {
        ...
        "I007": ("C-M107-09", "S-M107-01", "注記『食品トレー（発泡スチロール）はスーパーなどの回収容器へ』", "白色の発泡スチロール製食品トレー", "汚れを落として回収へ回す", "弁当等の非発泡食品トレーは燃えるごみ", "DIRECT_ITEM", "現行ポスターが素材を区別して店頭回収を明示。通常収集へ誤投影しない。"),
        ...
        "I031": ("C-M107-08", "S-M107-04", "分別一覧『電球』『LED電球』の行", "家庭用の電球・LED電球", "割れないように出す", "蛍光管も有害・危険ごみ", "DIRECT_ITEM", "現行公式サイト掲載中の品目別ガイドで補完。"),
        "I033": ("C-M107-08", "S-M107-04", "分別一覧『使い捨てライター』『ライター（使い捨て）』の行", "中身を使い切った使い捨てライター", "中身を使い切って出す", "金属製ライターは燃えない粗大ごみ", "DIRECT_ITEM", "現行公式サイト掲載中の品目別ガイドで補完。"),
    },
    "M108": {
        "I001": ("C-M108-09", "S-M108-01", "7頁 ペットボトル欄", "PETマークのある飲料・特定調味料用ボトル", "キャップとラベルを外し汚れを落とす", "汚れが落ちないものは普通ごみ", "DIRECT_ITEM", "令和8年度公式冊子の専用区分を採用。"),
        "I007": ("C-M108-11", "S-M108-01", "7頁 白色トレイ欄", "白色の発泡スチロール製食品トレイ", "汚れを落として出す", "色柄物・汚れた物は普通ごみ又は店頭回収の条件に従う", "DIRECT_ITEM", "令和8年度公式冊子の専用区分を採用。"),
        "I013": ("C-M108-03", "S-M108-01", "5頁 有価物『新聞・雑誌・雑がみ』欄", "家庭から出る新聞・折込広告", "ひもでしばって出す", "汚れた紙は普通ごみ", "DIRECT_ITEM", "公式子区分を採用。"),
        "I004": ("C-M108-06", "S-M108-01", "5頁 有価物『ビン・缶』欄", "飲料・食品用のアルミ缶", "中身を空にする", "スプレー缶は中身を使い切る", "DIRECT_ITEM", "公式子区分を採用。"),
        "I006": ("C-M108-06", "S-M108-01", "5頁 有価物『ビン・缶』欄", "割れていない飲料・食品用ガラスびん", "ふたを外し中身を空にする", "割れたびんは埋立ごみ", "DIRECT_ITEM", "公式子区分を採用。"),
        "I031": ("C-M108-07", "S-M108-01", "6頁 埋立ごみ欄『白熱球（LED含む）』", "白熱電球・LED電球", "割れないよう安全に出す", "蛍光管は有害ごみ", "DIRECT_ITEM", "令和8年度公式冊子が白熱球とLEDを明記。"),
        "I029": ("C-M108-08", "S-M108-01", "6頁 有害ごみ欄『モバイルバッテリー』", "家庭用モバイルバッテリー", "電極をテープで絶縁する", "販売店等への返却も案内される", "DIRECT_ITEM", "町の通常有害ごみ区分へ安全に投影可能。"),
        "I014": ("C-M108-04", "S-M108-01", "5頁 有価物『ダンボール』欄", "家庭から出るダンボール", "たたんでひもでしばる", "汚れたものは普通ごみ", "DIRECT_ITEM", "公式子区分を採用。"),
        "I033": ("C-M108-01", "S-M108-01", "4頁 普通ごみ欄『ライター』", "中身を使い切った使い捨てライター", "中身を使い切るかガスを抜いて出す", "中身が残る状態では出さない", "DIRECT_ITEM", "令和8年度公式冊子の品目例を採用。"),
        "I017": ("C-M108-10", "S-M108-01", "7頁 紙パック欄", "内側が白い紙パック", "ストロー等を外し洗って出す", "アルミ箔付きは普通ごみ", "DIRECT_ITEM", "令和8年度公式冊子の専用区分を採用。"),
    },
    "M109": {
        ...
        "I031": ("C-M109-02", "S-M109-01", "2頁 50音一覧『電球（蛍光管を除く）』", "蛍光管を除く家庭用電球", "割れないよう安全に出す", "蛍光管は有害ごみ", "DIRECT_ITEM", "令和8年度公式PDFの50音一覧を採用。"),
        "I033": ("C-M109-03", "S-M109-01", "2頁 50音一覧『ライター（使い切る）』", "中身を使い切った使い捨てライター", "中身を使い切って出す", "中身が残る状態では出さない", "DIRECT_ITEM", "令和8年度公式PDFの50音一覧を採用。"),
    },
}
...
def main() -> None:
    ...
    print(
        "Item image mapping pilot written: 90 decisions (90 VERIFIED, 0 UNRESOLVED); "
