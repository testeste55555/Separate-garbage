...
TARGETS={'M064','M066','M067','M068','M069','M070','M071','M072','M073'}
...
def main():
 ...
 checks.append(('deferred Chibu absent from Batch 07',all(r.get('municipality_id')!='M065' for dataset in (munis,cats,qa,cov,ev) for r in dataset),''))
 checks.append(('all nine active municipalities QA_PASSED',all(q[mid]['確認ステータス']=='QA_PASSED' for mid in TARGETS),''))
 checks.append(('reviewed leaf counts match evidence-backed design',all(counted_category_total(mid,cats)==EXPECTED[mid] and int(by[mid]['reviewed_category_count'])==EXPECTED[mid] and evc[mid]>=1 for mid in TARGETS),str({mid:counted_category_total(mid,cats) for mid in sorted(TARGETS)})))
