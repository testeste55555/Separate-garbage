#!/usr/bin/env python3
"""Adversarial checks for Batch 08 resident-facing semantics."""
...
def main():
 ...
 checks.append(('nine fully reviewed municipalities pass QA',all(q[mid]['確認ステータス']=='QA_PASSED' for mid in PASS),''))
 checks.append(('Bizen alone remains QA_REQUIRED',q['M076']['確認ステータス']=='QA_REQUIRED',q['M076']['確認ステータス']))
 checks.append(('Bizen is NOT_REVIEWED despite storing 23-division evidence',by['M076']['category_count_check_status']=='NOT_REVIEWED' and by['M076']['category_count_verified']=='FALSE' and counted_category_total('M076',cats)==23,''))
 checks.append(('Bizen keeps district scope on all 23 active divisions',all('資源回収ステーション設置済地区' in r.get('適用条件','') for r in cats if r['municipality_id']=='M076' and r.get('rule_status')=='CURRENT' and r.get('ui_role')!='EXCLUDED_NOTICE'),''))
 checks.append(('Bizen records coexistence with old-division districts','旧分別' in by['M076']['備考'] and '9種23分別' in by['M076']['備考'],by['M076']['備考']))
 checks.append(('reviewed leaf counts match evidence',all(counted_category_total(mid,cats)==EXPECTED[mid] and (by[mid]['official_category_count']=='16' if mid=='M079' else int(by[mid]['reviewed_category_count'])==EXPECTED[mid]) and evc[mid]>=1 for mid in PASS),str({mid:counted_category_total(mid,cats) for mid in sorted(PASS)})))
