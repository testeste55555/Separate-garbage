...
- `item_image_mapping_pilot_top8.csv`：歴史的ファイル名を維持した、画像10品目とactive 9自治体の公式分別先Pilot台帳（90 VERIFIED / 0 UNRESOLVED）
...
後者は40品目`APP_READY`への昇格ではなく、残り30品目を未完了のまま明示します。対面授業モードでは画像問題を使用せず、固定10品目専用BOXではなく授業で必要な主要分別箱を投影します。`SIMPLIFIED_ACTION`は自治体正式区分ではなく、非通常収集品目等を通常分別BOXへ誤投影せず自動採点するための教材用簡略行動箱です。M106のI029とM107のI007は詳細な非通常経路をエビデンス層に保持し、学習者には「回収・確認」だけを表示します。
