...
学習者画像仕分けPilotはオンライン授業モードでのみ`VERIFIED`行を画像問題として
読み込み、公式下位区分をcategory正本のCURRENT `SORT_BUCKET`へ投影します。
対面授業モードは分別箱のみを表示します。ここでいうオンライン／対面は授業形態で、
端末のインターネット接続状態ではありません。

`UNRESOLVED`はUIの正誤判定へ渡しません。品目名・条件・出す前の処理・条件外の扱いは
教師用データとして保持し、学習者の画像問題には表示しません。
